...
def register():
    ...
    if registerForm.validate_on_submit():
        ...
        plain_password = registerForm.password.data
        print(name, email, plain_password)
        if db.session.execute(db.select(User).where(User.email == email)).scalar():
            flash("The user already exist, login instead")
            return redirect(url_for('login'))

        enctrypted_pass = generate_password_hash(plain_password, method="pbkdf2:sha256", salt_length=8)
        new_user = User(
            email = email, 
            name = name, 
            password = enctrypted_pass
        )
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user and check_password_hash(user.password, password):
            ...
            flash("The email or password don't match. Try again")
            return redirect(url_for('login'))
...
def show_post(post_id):
    ...
    if comment_form.validate_on_submit():
        if not current_user.is_authenticated:
            flash("You need to login to comment on the post")
            return redirect(url_for('login'))
